...
* init
 ...
 =early-init.el= 是 Emacs 启动时最开始执行的文件，由于很多包还没有加载，执行复杂逻
 辑可能导致 Emacs 启动时悄无声息失败，所以该文件尽量以变量定义为主。
...
软件源仓库，用于为 straight.el 提供包列表和包来源信息：
...
=exec-path-from-shell= 将 shell 环境变量拷贝到 Emacs 环境中，解决命令找不到的问题。
#+begin_src emacs-lisp
