...
#+DATE: 2023-08-20 Sun
#+LASTMOD: 2026-09-14T20:40:07+0800
...
Emacs 31 内置 tree-sitter 模式切换和缺失 grammar 的安装提示。 =treesit-enabled-modes= 通过 Custom
setter 更新模式映射，不能只用 setq。
...
Emacs 31 的 buffer 在启用使用 tree-sitter 的 =XX-ts-mode= 以及启用 =hs-minor-mode= 后，才能按语法折叠。
...
** outline
:PROPERTIES:
:ID:       6F57658C-0F1C-4ED7-A317-984406A4F36A
:END:

使用 imenu-list 在一个 side-window 中显示当前 buffer 的标题、源码结构：
+ 使用 major-mode 的 Imenu 索引，不依赖 project 和 LSP。
+ 支持层级折叠、跳转、自动刷新。
...
执行 =M-x imenu-list-smart-toggle= 开关侧栏。在侧栏中：

• RET：跳转到条目。
• SPC：显示条目对应位置，焦点保留在侧栏。
• TAB：展开或折叠。
• g：手动刷新。
• q：关闭侧栏。

=imenu-list= 默认启用 Emacs 内置的 =hs-minor-mode（Hideshow）= ，因此可以直接使用它的折叠命令来按照级别、block 等方式折叠。

** clang
:PROPERTIES:
:ID:       B8B971AF-2F85-401D-A8DF-166059E57372
:END:
...
名会话追加 =-1=、 =-2= 等序号，避免覆盖。
预占的空文件在首次写入时初始化标准元信息头部；已有正文不会被重新初始化。
...
* ssh
:PROPERTIES:
:ID:       E6FD6910-0507-42F3-A0D3-5BAD49549B18
:END:

consult-dir 提供 ssh host 自动补全功能。

使用 tramp 成功 ssh 登录远程机器的注意事项：
- tramp 通过 ssh 登录时，如果需要账号密码，则默认从 =~/.authinfo.gpg= 中读取，该文件支持使用 =default=
  machine 名称来指定缺省账号密码。
- 在 =~/.ssh/config= 的 =Host *= 中关闭 host key check 和默认尝试的多个私钥（最多配置 3 个私钥，否则可能
  会导致认证失败超过 SSH Server 默认的 6 次，从而没有机会使用兜底的 password 认证。）
- 如果直接在终端中使用 ssh 登录，则可以使用 bash alias fj 来登录目标机器，如 =fj 230E= ,它使用 sshpass
  来提供默认密码。

避免打开远程文件、目录或终端卡住的做法：
- 远程 VC 排除、project-current 拦截、远程面板改用 consult-ghostel。
- 普通 Dired 图标仅本地启用。
- auto-revert-remote-files nil、Eglot 仅本地自动启动。
- recentf 排除远程文件，自动保存统一写本地目录。
- Marginalia：本机版本已跳过远程文件详细属性查询，无需全局关闭。
- ghostel-comint：此前确认没有接管 TRAMP 通信缓冲区，无需因此禁用。


为了为 ssh tramp 提供最佳性能，可以在 =~/.ssh/config= 的 =Host *= 中配置连接回话保持特性。
