...
#+end_src
+ early-init.el 是 Emacs 启动时最开始执行的文件，由于很多包还没有加载，执行复杂
  逻辑可能导致 Emacs 悄无声息失败，所以该文件尽量以变量定义为主。
...
straight.el 是 Emacs 内置的包管理器 package.el 的替代品。它使用 Git checkout +
build 机制来安装软件包（而非从 package archives 下载 tarballs 包）。
...
#+end_src
+ 关闭 company-snippets 候选提示，手动触发 snippets 补全（ =C-c s= ），否则提示太
  多的 snippets 候选项。
...
5. 部署生效,:
   ...
   + 对于 emacs-rime，如果修改了 =~/Library/Rime= 下的配置，需要执行 =M-x
     rime-deploy= 生效；
...
#+END_SRC
+ 对于 fundamental-mode 类型的 buffer, 可以使用 =M-s o(occur)= 或 =M-s
  l（consult-line)= 进行搜索、跳转。
...
# 在 web qq 中申请授权码, 作为帐号密码, 添加 qq 帐号
$ pass insert email/qq
$ pass insert email/gmail
#+end_src
+ 帐号信息保存在 =~/.password-store=
...
Host imap.qq.com
User geekard@qq.com
PassCmd "pass email/qq"  # 调用 pass 命令
Port 993
AuthMechs LOGIN
SSLType IMAPS
...
+ 如 mbsync 不感知任何代理环境变量, 通过 proxychains 可以代理。
...
mu4e 默认使用 gnus 发送 SMTP 邮件, 而 gnus 从 =~/.authinfo.gpg= 读取SMTP 服务器的
帐号信息:
...
#+end_src
+ minted 提供 tex pdf 中代码块语法高亮的功能(TexLive 默认按照), 需按照依赖: =brew
  install pygments=;
+ 变量 =org-latex-minted-langs= 列出 Emacs Major-Mode 与 minted 语言类型
  （pygmentize -L lexers）的关系, 如果两者一致, 如 go-[mod] 和 go, 则不需要列出。
+ minted 的 fontfamily 只对预定义的 tt/courier/helvetica 有效（参考 minted 的手
  册）。

安装 noto-cjk 中文字体: =git clone https://github.com/googlefonts/noto-cjk.git=

安装荧光笔效果的强调宏包 breakfbox:
+ 克隆 https://github.com/doraTeX/breakfbox 到
  =/usr/local/texlive/texmf-local/tex/latex=
+ 刷新数据库: =sudo mktexlsr=

自定义样式 mystyle.sty:
#+begin_src latex :tangle  ~/.emacs.d/mystyle.sty
% https://blog.shimanoke.com/ja/posts/change-latex-emph/
% 克隆 https://github.com/doraTeX/breakfbox 到 /usr/local/texlive/texmf-local/tex/latex
% 刷新数据库:  sudo mktexlsr
...
#+end_src
+ 查询示例： =@6-months-ago +unread !https://emacs-china.org +emacs=
+ 清理数据库： =M-x elfeed-db-unload; rm -rf ~/.emacs.d/elfeed=
+ 清理任务队列： =M-x elfeed-unjam=
+ 更新： =M-x elfeed-dashboard-update=
...
#+end_src
+ 默认将 OAuth Token 加密保存到 =~/.twittering-mode.gpg=, 第一次需要输入两次相同的
  加密密码。
...
+ [[https://emacstalk.github.io/post/007/][url-retrieve 使用 curl 作为后端实现]], 这样全局使用 socks5 代理。
...
+ 如果启动时报错，则执行命令 =rm ~/.emacs.d/elpa/engine-mode*/engine-mode-*.el*=;
...
#+end_src
+ =tramp-default-method= 缺省值为 scp, 不支持多跳（但拷贝大文件时性能更高），再打
  开多跳远程文件时每次都需要修改 /- 中的 - 为 ssh，较麻烦，所以设置为 ssh。
...
;; 显示 buffer 列表时会自动连接 TRAMP buffer, 可能会 hang.
