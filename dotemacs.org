...
#+LASTMOD: 2024-03-15T20:03:20+0800
...
orderless 补全风格：使用空格分割匹配模式，模式的顺序没有关系，但是 AND 关系。各模式可以使用如下几种
类型：
...
eglot 不指示 eldoc 在 echo-area 显示结构化成员(field) 或函数签名信息, 但是 M-x eldoc-doc-buffer(或快
捷键 C-h-.)  在单独的 eldoc buffer 中会显示这些信息.
...
eglot 启动后，将 xref-backend-functions 设置为 eglot-xref-backend，而忽略已注册的其它 backend，解决
办法是使用 .dir-local 文件关闭 eglot mode。
...
C/C++/Rust 语言调试: 
+ llvm 项目提供了名为 lldb-vscode(新版被重命名为 lldb-dap) 的 debug adapter, 但相比 codelldb 功能较
  弱而且和 dape 不兼容(执行失败), 会报类似错误: failed to load objfile
