...
使用 straight.el 来替代 Emacs 内置包管理器 package.el, 它使用 Git checkout +
build 机制来安装软件包（而非从 package archives 下载 tarball）。
...
使用 switchkey 程序将 Emacs 的系统输入法设置为英文: =brew install --cask
switchkey=
...
lsp-python-ms 和 pyls，而使用 lsp-pyright。yapf 用来格式化 python 代码。
...
Access Token，然后将 username 和 token 保存到 =~/.authinfo.gpg= 文件中：
...
#+end_src
+ 使用 web-mode 打开 json 文件，否则 prettier 会使用错误的 parser。
...
先安装 direnv 命令 =brew install direnv=, 然后[[https://direnv.net/docs/hook.html][将 direnv hook 到 shell 启动文件中]]：
#+begin_src shell :results none
