...
#+DATE: 2023-08-20 Sun
#+LASTMOD: 2024-09-28T16:09:00+0800
...
字体：英文字体 Iosevka/Sarasa 和中文字体 LxgwWenKai，按照 1:1 缩放，在偶数字号的情况下可以实现中英
文等宽等高。
...
自定义导出 PDF 的 LaTeX 样式 mystyle.sty: 对于表格，如果列内容过宽则导出的 PDF 中该列的内容会被截
断，可以为表格设置如下属性，将超宽列 align 设置为 X 来解决：
...
rust-analyzer 调用 rustfmt (rustup component add rustfmt) 来格式化代码，为代码添加
~#[rustfmt::skip]~ 可以忽略格式化。
