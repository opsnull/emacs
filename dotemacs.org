...
+ 安装 Noto Sans Symbols 和 Noto Sans Symbols2: https://fonts.google.com/noto
...
pyright _不使用_ pyenv 的 ~.python-version~ 指定的 python 版本或 venv，需要在项目 =pyrightconfig.json= 文件中配置
venv 和 venvPath 参数来指定 python 环境：
+ venvPath：指定查找 venv 目录的上级目录，可以包含多个 venv 环境；
+ venv：指定 venvPath 目录下的、使用的虚拟环境名称, pyright 在 venv 中搜索 pip 安装的包;
...
(use-package deadgrep
  ...
  :bind ("<f5>" . deadgrep))
#+end_src
+ 不启用 (treemacs-tag-follow-mode t), 因为启用后每次 treemacs 只会自动显示一个 project, 对于 Go 等有多个
  module project的情况不友好。不启用后，可以按需手动将 Go 依赖的 module project 加到当前 workspace 中。
