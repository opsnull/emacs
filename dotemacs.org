...
#+end_src
+ "epa--decode-coding-string not defined"的报错解决办法参考上面的 defalias;
+ 默认将 OAuth Token 加密报错到 "~/.twittering-mode.gpg", 第一次需要输入两次加密
  密码。
...
+ [[https://emacstalk.github.io/post/007/][url-retrieve 使用 curl 作为后端实现]], 这样全局可以使用 socks5 代理。 下载自：
  https://github.com/jiacai2050/mb-url,需要添加 =--user-agent= 配置, 否则会被
