...
#+LASTMOD: 2022-02-19T23:02:01+0800
#+Options: toc:nil h:4
...
cape 为 corfu 提供多种类型的 Completion At Point Extensions.
#+begin_src emacs-lisp
(use-package cape
  :demand
...
#+end_src
+ 不开启 org-journal-enable-agenda-integration, 而是向 org-agenda-files 变量添加日志文件的方式。否则 在历史日
...
+ 不启用 (treemacs-project-follow-mode t), 否则每次 treemacs 只会显示一个 project, 对于 Go 等有多个 module
  project的情况不友好。不启用后，可以按需手动将 Go 依赖的 module project 加到当前 workspace 中。
