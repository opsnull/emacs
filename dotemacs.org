...
direnv 是全局生效的，而 envrc 是 buffer-local 生效，所以切换到 envrc：
#+begin_src emacs-lisp
(use-package envrc
...
#+end_src
+ tree-sitter-hl-mode would override the font-lock-based highlighting provided
  by the major mode (python-mode/rust-mode).
+ Flycheck and other linting packages are unaffected, as they don't rely on
  font-lock-mode.
* 项目管理
