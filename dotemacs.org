...
#+LASTMOD: 2024-04-02T20:04:47+0800
...
安装 [[https://github.com/rime/librime/releases][librime]] 库, 它是 RIME 输入法后端引擎：
...
上面的步骤也适合于 userdb 文件损坏导致的个人词频不生效的情况。
...
+ xref-backend-functions 可能会被添加多个 backend，它使用第一个 backend 返回地信息，所以必须确保
  ...
  + dump-jump 会添加到 citre-xref-backend 前面。
