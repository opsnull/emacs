...
使用 =use-package + straight= 替代 Emacs 内置的 =package.el=, 它使用 Git checkout + build 机制来安装软件包（而非从包仓库下
载）：
...
=exec-path-from-shell= 将用户 Shell 环境变量拷贝到 Emacs 环境中，解决找不到命令的问题：
#+begin_src emacs-lisp
...
#+end_src
+ =(setq auto-revert-check-vc-info t)= 可以自动 revert buffer，确保 modeline 上的分支名正确，但是 CPU Profile 显示比较影响
  性能，故暂不开启。
...
(use-package treemacs-projectile :after (treemacs projectile) :disabled t)
#+end_src
