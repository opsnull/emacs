...
(use-package eglot
  :hook ((c-mode go-mode java-mode js-mode python-mode rust-mode web-mode) . eglot-ensure)
  :config
  ...
  ;; (add-hook 'go-mode-hook #'eglot-actions-before-save)
...
(setq-default eglot-workspace-configuration
    '((:gopls .
        ((staticcheck . t)
         ...
         (experimentalUseInvalidMetadata . t)))))
#+end_src
...
doom-modeline 总存在 modeline 右侧溢出的情况。
#+begin_src emacs-lisp
;; modeline 显示电池和日期时间。
...
(use-package doom-modeline
  ...
  :config
  (set-face-attribute 'mode-line nil :font "Fira Code Retina" :height 0.9)
  (set-face-attribute 'mode-line-inactive nil :font "Fira Code Retina" :height 0.9))
#+end_src
...
gopls 自带了诊断功能，所以不再需要：
#+begin_src emacs-lisp
;; golangci-lint 性能较 go-xxx 工具高，同时避免调用过多 checker.
...
tree-sitter 与中文输入法连用时，会导致输入过程变慢，同时输入过程中 buffer 的高亮显式异常。
#+begin_src emacs-lisp
