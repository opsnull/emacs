...
(use-package citre
  ...
  (setq citre-auto-enable-citre-mode-modes '(c-mode c-ts-mode go-mode go-ts-mode))
...
eglot-xref-backend。
+ 解决办法：定义一个函数 my/toggle-eglot 来关闭 eglot，这时 xref-backend-functions 会恢复为 eglot 启
  动前设置的值。
